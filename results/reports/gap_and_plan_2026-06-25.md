...
## #3 窄 N — 找到便宜的调度修复（blocking_probe 实测）
`huawei_blocking_probe.sh` 发现：窄 N 形状关掉 packed 2D-grid（`PACK2D=0`，退化为纯 M-split）
华为实测 **+51–97%**：`2048×4096×64 @t48` 4573→9024、`2048×16384×24 @t64` 3153→5649。根因：
N-tile 太少时 pick_grid 被迫选 tall-pm × thin-pn，把 N 切成细列条 + 行维负载不均。
**修复**：packed 2D-grid 触发条件加 `n_tiles >= 8`（窄 N 退回 M-split；t79 rescue 形状 N 大、
n_tiles≥64 不受影响）。commit 见下，6000+7168 用例过，待华为复测确认默认路径吃到 +51–97%。
中等过度并行/大 N 形状 pack2d0 仅 ±2–3%（噪声内），不动。
