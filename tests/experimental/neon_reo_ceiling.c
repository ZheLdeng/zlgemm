...
void i8_pack_A_neon_m8_asm(const i8_t*,i8_t*,int,int);
void i8gemm_k_reo_ld(const i8_t*,const i8_t*,i32_t*,i8_t*,const gemm_params_t*);
...
int main(int argc,char**argv){
    ...
    i8_t*B=aa((size_t)Kr*Nr),*Br=aa((size_t)Kr*Nr),*Af=aa((size_t)M*Kr),*Ar=aa((size_t)M*Kr+4096);
    i32_t*C=aa((size_t)M*Nr*sizeof(i32_t)*4);
    for(int i=0;i<M*Kr;i++)Af[i]=(i%17)-8;
    for(int i=0;i<Kr*Nr;i++)B[i]=(i%13)-6;
    i8_pack_B(B,Br,Kr,Nr); i8_pack_A_neon_m8_asm(Af,Ar,Kr,Kr);
    gemm_params_t p={M,Kr,Nr,Kr,Kr,Nr};
    long reps=400000;
    for(int w=0;w<1000;w++)i8gemm_k_reo_ld(Af,Br,C,Ar,&p);
    double best=1e30;for(int r=0;r<5;r++){double t=ns();for(long j=0;j<reps;j++)i8gemm_k_reo_ld(Af,Br,C,Ar,&p);double d=ns()-t;if(d<best)best=d;}
    double g=2.0*M*K*N*(double)reps/best/1e9;
    printf("NEON reo prepacked (M8 N%d K%d in-L1): %.1f GOPS = %.1f%% of peak(661)\n",N,K,g,g*100/661.22);
    return 0;
}
